\chapter{VOLTAGE AND CURRENT VARIATION IN LOW-LOSS TRANSMISSION LINE}
...
\begin{enumerate}[(i)]
	\item To know the conditions necessary for treating the transmission line as lowloss at a particular frequency.
...
Similarly, substituting $\gamma = j\beta$ and $x = -l$ into equation~\eqref{eqn:currentfromloadlec5}:
\begin{align*}
...
Using the negative and positive sign will both give correct answers. Here, we will use the negative sign, that is, $l_\max-\frac{\lambda}{4}$. When $m = 1, 2, 3, \ldots$, therefore $|I|_\min$ occurs at $0.16\lambda , 0.66\lambda , 1.16\lambda$ and so on.
\end{exmp}
...
\label{eqn:minimp}
\end{dmath}
Once load impedance, characteristic impedance $Z_0$ as well as the reflection coefficient $\Gamma_L$ are known, then we can calculate the maximum and minimum value of impedance that we see on the transmission line. As we move along the transmission line, the impedance will vary. However, there is a bound on the upper and lower value of this impedance; the lowest according to equation~\eqref{eqn:maximp} and the highest according to equation~\eqref{eqn:minimp}.
...
\Exercise[label={ex56}]
What is the significance of the surge impedance loading in a lossless transmission line?
...
\Exercise[label={ex60}]
Explain how the absence of resistive losses influences the phase velocity of signals in a lossless transmission line.
\end{ExerciseList}
