...
\begin{ExerciseList}
	\Exercise[label={ex11}] How does real characteristic impedance and real propagation constant impact our ability to determine whether the line is lossy or lossless?
	\Exercise[label={ex12}] On a moderately lossy transmission line, how much does the reflection coefficient at the load point affect the standing wave's amplitude variation?
	\Exercise[label={ex13}]In what way does the purely imaginary propagation constant in a lossless transmission line differ from the real propagation constant in a very lossy line and what implication does this have for the representation of a wave phenomena in the transmission line?
	\Exercise[label={ex14}] How is wave propagation affected by the medium's properties, i.e. density and elasticity?
	\Exercise[label={ex15}] What is the relationship between  properties of a medium and the velocity of a wave?
	\Exercise[label={ex16}] How does wave interaction with the medium affect its propagation?
	\Exercise[label={ex17}] What is the Skin depth of a material and how does it affect wave propagation?
	\Exercise[label={ex18}] Under what factors does a wave lose energy or attenuate when propagating through a medium?
	\Exercise[label={ex19}] What is the relationship between Refractive Index and dielectric constant?
	\Exercise[label={ex20}] What is a pure dielectric and how does it differ from a conductor?
	\Exercise[label={ex21}] What is a dispersive medium and why is it called a dispersive medium?
	\Exercise[label={ex22}] What is the requirement to have good wave propagation in a conductor?
	\Exercise[label={ex23}]  What is the difference between wave propagation in a dielectric medium and a conducting medium?
\end{ExerciseList}
\end{mdframed}
