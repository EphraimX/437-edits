...
\begin{ExerciseList}
	...
	_{}\textbf{Solution:}
%	To apply Ampere's Circuital Law, consider a circular path in the $yz$-plane centered at the center of the loop. Parameterize the path as $\mathbf{r}'(t) = \begin{bmatrix} 0 \\ R\sin t \\ R\cos t \end{bmatrix}$ for $0 \leq t \leq 2\pi$, where $R$ is the distance from the center of the loop to any of its sides.
%	
%	The magnetic field $\mathbf{B}$ can be calculated using the formula
%	\[
%	\mathbf{B} = \frac{\mu_0}{4\pi} \int_C \frac{I\,d\mathbf{l} \times (\mathbf{r} - \mathbf{r'})}{|\mathbf{r} - \mathbf{r'}|^3},
%	\]
%	where $C$ is the path of integration, $I$ is the current, $\mathbf{r}$ is the position vector of the point where the field is being calculated, and $\mathbf{r'}$ is a vector along the path of integration.
%	
%	Compute the cross product $\mathbf{I}\,d\mathbf{l} \times (\mathbf{r} - \mathbf{r'})$ and evaluate the integral to find the magnetic field.
%	
%	The final magnetic field can be expressed in terms of the unit vectors $\mathbf{i}$, $\mathbf{j}$, and $\mathbf{k}$ as $B_x \mathbf{i} + B_y \mathbf{j} + B_z \mathbf{k}$, where $B_x$, $B_y$, and $B_z$ are the components of the magnetic field.
...
%	\textbf{Solution:}
%	To apply Ampere's Circuital Law, consider a circular path in the $yz$-plane centered around the wire. Parameterize the path as $\mathbf{r}'(t) = \begin{bmatrix} 0 \\ R\sin t \\ R\cos t \end{bmatrix}$ for $0 \leq t \leq 2\pi$, where $R$ is the radius of the circle.
%	
%	The magnetic field $\mathbf{B}$ can be calculated using the formula
%	\[
%	\mathbf{B} = \frac{\mu_0}{4\pi} \int_C \frac{I\,d\mathbf{l} \times (\mathbf{r} - \mathbf{r'})}{|\mathbf{r} - \mathbf{r'}|^3},
...
%	The final magnetic field can be expressed in terms of the unit vectors $\mathbf{i}$, $\mathbf{j}$, and $\mathbf{k}$ as $B_x \mathbf{i} + B_y \mathbf{j} + B_z \mathbf{k}$, where $B_x$, $B_y$, and $B_z$ are the components of the magnetic field.
	...
	\Exercise[label={ex19}] Explain Boundary conditions.
\end{ExerciseList}
\end{mdframed}